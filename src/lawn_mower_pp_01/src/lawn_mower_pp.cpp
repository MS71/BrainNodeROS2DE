...
#include "lawn_mower_pp/lawn_mower_pp.hpp"
...
#include "lawn_mower_pp/navfn.hpp"
...
namespace lawn_mower_pp_01
{

LawnMowerPP::LawnMowerPP()
...
{
  RCLCPP_INFO(
    node_->get_logger(), "Destroying plugin %s of type LawnMowerPP",
    name_.c_str());
}

void
LawnMowerPP::configure(
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  RCLCPP_INFO(
    node_->get_logger(), "Configuring plugin %s of type LawnMowerPP",
    name_.c_str());
...
void
LawnMowerPP::activate()
{
  RCLCPP_INFO(
    node_->get_logger(), "Activating plugin %s of type LawnMowerPP",
    name_.c_str());
}

void
LawnMowerPP::deactivate()
{
  RCLCPP_INFO(
    node_->get_logger(), "Deactivating plugin %s of type LawnMowerPP",
    name_.c_str());
}
...
{
  RCLCPP_INFO(
    node_->get_logger(), "Cleaning up plugin %s of type LawnMowerPP",
    name_.c_str());
  planner_.reset();
}

nav_msgs::msg::Path LawnMowerPP::createPlan(
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  steady_clock::time_point a = steady_clock::now();
    ...
    RCLCPP_WARN(
      node_->get_logger(), "%s: createPlan", name_.c_str() );
...
bool
LawnMowerPP::isPlannerOutOfDate()
{
  if (!planner_.get() ||
...
void
LawnMowerPP::smoothApproachToGoal(
...
bool
LawnMowerPP::getPlanFromPotential(
  const geometry_msgs::msg::Pose & goal,
  nav_msgs::msg::Path & plan)
{
  // clear the plan, just in case
...
double
LawnMowerPP::getPointPotential(const geometry_msgs::msg::Point & world_point)
{
  unsigned int mx, my;
...
// bool
// LawnMowerPP::validPointPotential(const geometry_msgs::msg::Point & world_point)
// {
//   return validPointPotential(world_point, tolerance_);
// }

// bool
// LawnMowerPP::validPointPotential(
...
bool
LawnMowerPP::worldToMap(double wx, double wy, unsigned int & mx, unsigned int & my)
{
...
void
LawnMowerPP::mapToWorld(double mx, double my, double & wx, double & wy)
{
...
void
LawnMowerPP::clearRobotCell(unsigned int mx, unsigned int my)
{
...
PLUGINLIB_EXPORT_CLASS(lawn_mower_pp_01::LawnMowerPP, nav2_core::GlobalPlanner)
